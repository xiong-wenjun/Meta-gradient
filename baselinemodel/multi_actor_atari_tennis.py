...
# ====== 修改 2: 调整 IMPALA 超参 ======
NUM_ACTORS = 80                 # Tennis 较难，建议增加并行 Actor 数量 (如果 CPU 允许)
UNROLL_LENGTH = 20             
BATCH_UNROLLS = 32              # 增加 Batch Size 以稳定梯度 (16 -> 32)
MAX_FRAMES = 20_000_000         # Tennis 收敛较慢，建议增加到 20M 或 50M
GAMMA = 0.99

# ====== 修改 3: 降低学习率 ======
# Tennis 训练初期容易不稳定，LR 建议从 1e-3 降到 5e-4 或 6e-4
LR = 6e-4                      
ENTROPY_COEF = 0.01             
VALUE_COEF = 0.5
CLIP_GRAD_NORM = 40.0            # 稍微收紧梯度裁剪 (5.0 -> 1.0) 以防梯度爆炸

FRAME_SKIP = 4                  

# ---------------------------
#  辅助 Wrapper: 自动发球 (Fire Reset)
# ---------------------------
class FireResetWrapper(gym.Wrapper):
    """
    Tennis 等游戏在 reset 后需要执行 'FIRE' 动作才能开始游戏 (发球)。
    如果不加这个，Agent 可能很久都学不会开始游戏。
    """
...
def make_atari_env(env_id, seed=None):
    ...
    env = gym.wrappers.AtariPreprocessing(
        ...
        noop_max=30      # 随机 No-op 增加初始状态随机性
    )

    # ====== 修改 4: 挂载 FireResetWrapper ======
...
class ImpalaCNN(nn.Module):
    def __init__(self, input_channels, action_dim):
        ...
        # ====== 自动适应动作空间 ======
        # Pong 是 6 个动作，Tennis 是 18 个动作
        # 代码会自动根据传入的 action_dim 调整输出层大小
...
def actor_process(rank, env_id, unroll_length, data_queue, param_queue, seed):
    ...
    action_dim = env.action_space.n  # Tennis 这里会自动变成 18
...
def evaluate(env_id, agent, episodes=5):
    # 评估时也要带上 FireResetWrapper，否则也会发不出球
